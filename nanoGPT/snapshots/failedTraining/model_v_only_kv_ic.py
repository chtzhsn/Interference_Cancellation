...
This version adds a first-step, V-only KV-based interference-cancellation (VIC)
surrogate for teacher-forcing training:

- only the last transformer block is VIC-enabled
- only V is corrected, K is left unchanged
...
This is intended as the safest first implementation before moving to true
cache-edit / step-by-step inference-time KV rewriting.
"""
...
class ValueInterferenceCorrection(nn.Module):
    """
    V-only interference correction module.
    ...
    Output:
        corrected v_token, along with delta/gate for logging
    """

    def __init__(self, config):
        super().__init__()
        c = config.n_embd
        hidden = config.vic_hidden_dim if config.vic_hidden_dim > 0 else 2 * c
    ...
    def forward(self, v_token, e_selected, e_unselected, alpha):
        z = torch.cat([v_token, e_selected, e_unselected], dim=-1)
        delta_v = self.delta_mlp(z)
        gate_v = self.gate(z)
        v_corr = v_token + alpha * self.dropout(gate_v * delta_v)
        return v_corr, delta_v, gate_v
...
class CausalSelfAttention(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.use_vic = config.use_vic
        self.vic_alpha = config.vic_alpha
        self.vic_module = ValueInterferenceCorrection(config) if config.use_vic else None
    ...
    def forward(self, x, vic_info=None, return_vic_stats=False):
        B, T, C = x.size()
        ...
        if self.use_vic and vic_info is not None:
            # reassemble value tokens to (B, T, C), correct them, then split back
            v_token = v.transpose(1, 2).contiguous().view(B, T, C)
            e_selected = vic_info["e_selected"]
            e_unselected = vic_info["e_unselected"]

            v_corr_token, delta_v, gate_v = self.vic_module(
                v_token, e_selected, e_unselected, alpha=self.vic_alpha
            )
...
class Block(nn.Module):
    ...
    def forward(self, x, vic_info=None, return_vic_stats=False):
        if return_vic_stats:
            attn_out, vic_stats = self.attn(self.ln_1(x), vic_info=vic_info, return_vic_stats=True)
            x = x + attn_out
            x = x + self.mlp(self.ln_2(x))
            return x, vic_stats
        else:
            x = x + self.attn(self.ln_1(x), vic_info=vic_info)
            x = x + self.mlp(self.ln_2(x))
            return x
...
class GPTConfig:
    ...
    # ---- V-only KV-based IC options ----
    use_vic: bool = False
    vic_hidden_dim: int = 0
    vic_alpha: float = 0.1
    vic_lambda_base: float = 1.0
    vic_lambda_ic: float = 0.3
    vic_lambda_margin: float = 0.5
    vic_margin_target: float = 0.2
...
class GPT(nn.Module):
    ...
    def _compute_probe_stats(self, base_logits, vic_logits, targets):
        with torch.no_grad():
            valid = targets != -1
            ...
            vic_target = self._gather_logits(vic_logits, safe_targets)
            vic_comp = self._gather_logits(vic_logits, competitor_ids)
            ...
            vic_pred = torch.argmax(vic_logits, dim=-1)
    ...
    def _compute_margin_loss(self, logits, targets):
        ...
        losses = F.relu(self.config.vic_margin_target - margins)
    ...
    def _run_last_block_vic(self, x, e_selected, e_unselected):
        vic_info = {
            "e_selected": e_selected,
            "e_unselected": e_unselected,
        }
        x_vic, vic_stats = self.transformer.h[-1](x, vic_info=vic_info, return_vic_stats=True)
        return x_vic, vic_stats
    ...
    def forward(self, idx, targets=None, return_aux=False):
        ...
        if targets is None or not self.config.use_vic:
            ...
            if targets is not None and not self.config.use_vic:
                self.last_vic_stats = {
        ...
        # ---- training/eval VIC surrogate path ----
        ...
        # 4) V-only corrected path through last block
        x_vic, vic_internal = self._run_last_block_vic(x_prefix, e_selected, e_unselected)
        x_vic = self.transformer.ln_f(x_vic)
        vic_logits = self.lm_head(x_vic)
        vic_loss = self._masked_cross_entropy(vic_logits, targets)
        margin_loss = self._compute_margin_loss(vic_logits, targets)

        total_loss = (
            self.config.vic_lambda_base * base_loss
            + self.config.vic_lambda_ic * vic_loss
            + self.config.vic_lambda_margin * margin_loss
        )

        probe_stats = self._compute_probe_stats(base_logits, vic_logits, targets)

        self.last_vic_stats = {
            "base_loss": float(base_loss.detach().item()),
            "vic_loss": float(vic_loss.detach().item()),
            "margin_loss": float(margin_loss.detach().item()),
            "delta_v_norm": float(vic_internal["delta_v_norm"].detach().item()),
            "gate_v_mean": float(vic_internal["gate_v_mean"].detach().item()),
        }
        ...
        if return_aux:
            aux = {
                ...
                "vic_logits": vic_logits,
                "selected_ids": selected_ids,
                "unselected_ids": unselected_ids,
                "vic_stats": vic_internal,
                "probe_stats": probe_stats,
            }
            return vic_logits, total_loss, aux

        return vic_logits, total_loss
    ...
    def from_pretrained(cls, model_type, override_args=None):
        ...
        # Only copy overlapping parameters; VIC-specific weights stay randomly initialized
        for k in sd_keys_hf:
            if k not in sd:
                continue

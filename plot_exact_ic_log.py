"""
plot_exact_ic_log.py

Parse exact IC training log and generate report figures.

Expected input:
    logs/exact_ic_run.txt

Run from project root:
    python plot_exact_ic_log.py

Outputs:
    plots/exact_ic_loss.png
    plots/exact_ic_accuracy.png
    plots/exact_ic_kv_delta_norm.png
    plots/exact_ic_gate_mean.png
    plots/exact_ic_loss_gap.png
    plots/exact_ic_accuracy_gap.png
"""
...
LOG_PATH = Path("logs/exact_ic_run.txt")
OUT_DIR = Path("plots/v2.1_exact_ic")
...
def plot_loss(rows, eval_rows):
    ...
    if eval_rows:
        ...
        train = [r["train_loss"] for r in eval_rows]
        val = [r["val_loss"] for r in eval_rows]
        plt.plot(steps, train, marker="s", linestyle="--", label="eval_train_loss")
        plt.plot(steps, val, marker="s", linestyle="--", label="eval_val_loss")

    plt.xlabel("Iteration")
    plt.ylabel("Loss")
    plt.title("Exact IC: Base vs IC Loss")
    plt.legend()
    plt.grid(True, alpha=0.3)
    savefig("exact_ic_loss.png")
...
def plot_accuracy(rows):
    ...
    plt.title("Exact IC: Base vs IC Top-1 Accuracy")
    plt.legend()
    plt.grid(True, alpha=0.3)
    savefig("exact_ic_accuracy.png")
...
def plot_kv_stats(rows):
    ...
    plt.xlabel("Iteration")
    plt.ylabel("Norm")
    plt.title("Exact IC: KV Correction Magnitude")
    plt.legend()
    plt.grid(True, alpha=0.3)
    savefig("exact_ic_kv_delta_norm.png")
    ...
    plt.title("Exact IC: Gate Activation")
    plt.legend()
    plt.grid(True, alpha=0.3)
    savefig("exact_ic_gate_mean.png")
...
def plot_gap(rows):
    ...
    plt.xlabel("Iteration")
    plt.ylabel("Loss Gap")
    plt.title("Exact IC: IC Loss Gap")
    plt.legend()
    plt.grid(True, alpha=0.3)
    savefig("exact_ic_loss_gap.png")
    ...
    plt.title("Exact IC: IC Accuracy Gap")
    plt.legend()
    plt.grid(True, alpha=0.3)
    savefig("exact_ic_accuracy_gap.png")
